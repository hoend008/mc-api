...
from schemas.schemas import Sop, UserInDB
...
def get_sop(current_user: UserInDB = Depends(get_current_user)) -> List[Sop]:
...
def get_sops_based_on_user(current_user: UserInDB = Depends(get_current_user)):
    with PostgresDatabase(DB_NAME, DB_USER, DB_PASSWORD, realdictcursor=True) as db:
